...
export default function WhatsAppFloat() {
  // Prevent SSR mismatch by rendering only on client
  ...
  useEffect(() => {
    ...
    const animationCycle = () => {
      ...
      const blinkTimer = setTimeout(() => setShouldBlink(true), 5000);
      ...
      const hideTimer = setTimeout(() => {
        ...
        // Show again after 4 seconds hidden
        const showAgain = setTimeout(() => {
          setIsVisible(true);
          setShouldBlink(false);
        }, 4000);

        return () => clearTimeout(showAgain);
      }, 10000);
    ...
    const initialTimer = setTimeout(() => {
      ...
      // Repeat cycle every 16 seconds
      const interval = setInterval(animationCycle, 16000);
  ...
  const handleTouchStart = () => setShowText(prev => !prev);
